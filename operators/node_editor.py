...
from ..nodes.sockets import (
    SuperLuxCoreSocketColor, SuperLuxCoreSocketFloat, SuperLuxCoreSocketVector,
    SuperLuxCoreSocketMapping2D, SuperLuxCoreSocketMapping3D,
)
from .utils import poll_node_tree

superluxcore_viewer_reroute_mark = "superluxcore_viewer_reroute"
superluxcore_viewer_mark = "superluxcore_viewer"


def _remove_all_viewers(node_tree):
    for node in reversed(node_tree.nodes):
        if superluxcore_viewer_mark in node:
...
class SUPERLUXCORE_OT_node_editor_viewer(bpy.types.Operator):
    bl_idname = "superluxcore.node_editor_viewer"
    ...
    def invoke(self, context, event):
        ...
        for node in node_tree.nodes:
            if superluxcore_viewer_reroute_mark in node:
                viewer_node = node
                break
        ...
        if node_tree.bl_idname == "superluxcore_material_nodes":
            if active_node.bl_idname.startswith("SuperLuxCoreNodeMat"):
                ...
                if viewer_node:
                    ...
                    viewer_node[superluxcore_viewer_mark] = True
                    viewer_node[superluxcore_viewer_reroute_mark] = True
...
class SUPERLUXCORE_OT_mute_node(bpy.types.Operator):
    bl_idname = "superluxcore.mute_node"
    bl_label = "Mute Node"
...
class SUPERLUXCORE_OT_node_editor_add_image(bpy.types.Operator):
    bl_idname = "superluxcore.node_editor_add_image"
    bl_label = "Add Image"
    ...
    def invoke(self, context, event):
        ...
        allowed_inputs = {
            SuperLuxCoreSocketColor, SuperLuxCoreSocketFloat,
            SuperLuxCoreSocketMapping2D, SuperLuxCoreSocketMapping3D,
        }
        ...
        for node in [node for node in node_tree.nodes if node.select]:
            ...
            def is_2d_socket(socket):
                return isinstance(unlinked_input, SuperLuxCoreSocketMapping2D)
            def is_3d_socket(socket):
                return isinstance(unlinked_input, SuperLuxCoreSocketMapping3D)

            if is_2d_socket(unlinked_input) or is_3d_socket(unlinked_input):
                tex_node = node
            else:
                tex_node = node_tree.nodes.new("SuperLuxCoreNodeTexImagemap")
            ...
            if is_2d_socket(unlinked_input):
                mapping_node = node_tree.nodes.new("SuperLuxCoreNodeTexMapping2D")
                offset = Vector((-200, 0))
            else:
                assert is_3d_socket(unlinked_input)
                mapping_node = node_tree.nodes.new("SuperLuxCoreNodeTexMapping3D")
                offset = Vector((-300, 0))
