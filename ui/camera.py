...
class SUPERLUXCORE_CAMERA_PT_presets(PresetPanel, Panel):
...
class SUPERLUXCORE_SAFE_AREAS_PT_presets(PresetPanel, Panel):
    bl_label = "Camera Presets"
...
class SUPERLUXCORE_CAMERA_PT_lens(CameraButtonsPanel, Panel):
    bl_label = "Lens"
    bl_order = 1
    COMPAT_ENGINES = {"SUPERLUXCORE"}
    ...
    def draw(self, context):
        ...
        col.separator()
        col.prop(cam.superluxcore, "use_clipping")        
        sub = col.column(align=True)
        sub.active = cam.superluxcore.use_clipping
        sub.prop(cam, "clip_start", text="Clip Start")
        sub.prop(cam, "clip_end", text="End")


class SUPERLUXCORE_CAMERA_PT_clipping_plane(CameraButtonsPanel, Panel):
    ...
    def draw_header(self, context):
        ...
        col = layout.column(align=True)
        col.prop(context.camera.superluxcore, "use_clipping_plane", text="")
    ...
    def draw(self, context):
        ...
        sub.enabled = cam.superluxcore.use_clipping_plane
        sub.prop(cam.superluxcore, "clipping_plane", text="")


class SUPERLUXCORE_CAMERA_PT_depth_of_field(CameraButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        layout.prop(cam.superluxcore, "use_autofocus")

        col = layout.column(align=True)
        col.active = not cam.superluxcore.use_autofocus        
        col.prop(cam.dof, "focus_object")
        col = layout.column(align=True)
        col.active = (cam.dof.focus_object is None and not cam.superluxcore.use_autofocus)
        col.prop(cam.dof, "focus_distance", text="Distance")


class SUPERLUXCORE_CAMERA_PT_bokeh(CameraButtonsPanel, Panel):
    bl_label = "Non-Uniform Bokeh"
    bl_parent_id = "SUPERLUXCORE_CAMERA_PT_depth_of_field"
    bl_options = {'DEFAULT_CLOSED'}
    COMPAT_ENGINES = {"SUPERLUXCORE"}
    
    def draw_header(self, context):
        layout = self.layout
        layout.prop(context.camera.superluxcore.bokeh, "non_uniform", text="")
...
class SUPERLUXCORE_CAMERA_PT_motion_blur(CameraButtonsPanel, Panel):
    ...
    def draw_header(self, context):
        ...
        layout.label(text="", icon_value=icon_manager.get_icon_id("logotype"))
        col = layout.column(align=True)
        col.prop(context.camera.superluxcore.motion_blur, "enable", text="")

    def draw(self, context):
        layout = self.layout
        cam = context.camera
        motion_blur = cam.superluxcore.motion_blur
...
class SUPERLUXCORE_CAMERA_PT_image_pipeline(CameraButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        layout.use_property_split = True
        layout.use_property_decorate = False
        pipeline = context.camera.superluxcore.imagepipeline

        layout.prop(pipeline, "transparent_film")
            

class SUPERLUXCORE_CAMERA_PT_image_pipeline_tonemapper(CameraButtonsPanel, Panel):
    bl_label = "Tonemapper"
    bl_parent_id = "SUPERLUXCORE_CAMERA_PT_image_pipeline"
...
class SUPERLUXCORE_CAMERA_PT_image_pipeline_bloom(CameraButtonsPanel, Panel):
    bl_label = "Bloom"
    bl_parent_id = "SUPERLUXCORE_CAMERA_PT_image_pipeline"
    bl_options = {'DEFAULT_CLOSED'}
    COMPAT_ENGINES = {"SUPERLUXCORE"}    
...
class SUPERLUXCORE_CAMERA_PT_image_pipeline_mist(CameraButtonsPanel, Panel):
    bl_label = "Mist"
    bl_parent_id = "SUPERLUXCORE_CAMERA_PT_image_pipeline"
    bl_options = {'DEFAULT_CLOSED'}
    COMPAT_ENGINES = {"SUPERLUXCORE"}    
...
class SUPERLUXCORE_CAMERA_PT_image_pipeline_vignetting(CameraButtonsPanel, Panel):
    bl_label = "Vignetting"
    bl_parent_id = "SUPERLUXCORE_CAMERA_PT_image_pipeline"
    bl_options = {'DEFAULT_CLOSED'}
    COMPAT_ENGINES = {"SUPERLUXCORE"}    
...
class SUPERLUXCORE_CAMERA_PT_image_pipeline_color_aberration(CameraButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        if context.scene.superluxcore.viewport.get_denoiser(context) == "OIDN":
...
class SUPERLUXCORE_CAMERA_PT_image_pipeline_background_image(CameraButtonsPanel, Panel):
...
class SUPERLUXCORE_CAMERA_PT_image_pipeline_white_balance(CameraButtonsPanel, Panel):
...
class SUPERLUXCORE_CAMERA_PT_image_pipeline_camera_response_function(CameraButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        if camera_response_func.type == "PRESET":
            ...
            layout.operator("superluxcore.select_crf", text=label)
        else:
            layout.prop(camera_response_func, "file")


class SUPERLUXCORE_CAMERA_PT_image_pipeline_color_LUT(CameraButtonsPanel, Panel):
    bl_label = "LUT"
    bl_parent_id = "SUPERLUXCORE_CAMERA_PT_image_pipeline"
    bl_options = {'DEFAULT_CLOSED'}
    COMPAT_ENGINES = {"SUPERLUXCORE"}
    ...
    def draw(self, context):
        ...
        if (color_LUT.input_colorspace == "SRGB_GAMMA_CORRECTED"
            ...
            layout.operator("superluxcore.set_raw_view_transform")

        layout.prop(color_LUT, "file")
        layout.prop(color_LUT, "strength")


class SUPERLUXCORE_CAMERA_PT_image_pipeline_contour_lines(CameraButtonsPanel, Panel):
...
class SUPERLUXCORE_CAMERA_PT_volume(CameraButtonsPanel, Panel):
    ...
    def poll(cls, context):
        return context.scene.render.engine == "SUPERLUXCORE" and context.camera
    ...
    def draw(self, context):
        ...
        layout.prop(cam.superluxcore, "auto_volume")

        if not cam.superluxcore.auto_volume:
