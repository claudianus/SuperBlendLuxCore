...
def convert_uvs(
    ...
    if (
        settings.use_active_uv_map
        or settings.uv_map_name not in obj.data.uv_layers
    ):
    ...
    if engine:
        engine.update_stats(
            "Exporting...",
            "[%s: %s] Preparing %d UV coordinates"
...
def convert_colors(
    ...
    if (
        settings.use_active_vertex_color_layer
        or settings.vertex_color_layer_name not in vertex_colors
    ):
        active_vertex_color_layer = utils.find_active_vertex_color_layer(
            vertex_colors
        )
        if active_vertex_color_layer:
            vertex_color_index = vertex_colors.find(
                active_vertex_color_layer.name
            )
        else:
            vertex_color_index = -1
    else:
        vertex_color_index = vertex_colors.find(
            settings.vertex_color_layer_name
        )
    ...
    try:
        ...
        if num_children == 0:
            ...
            num_virtual_parents = math.trunc(
                0.3
                * psys.settings.virtual_parents
                * len(psys.child_particles)
                * num_parents
            )
        ...
        print(
            "Collecting Blender hair information took %.3f s"
            % (time() - collection_start)
        )
        if engine:
            engine.update_stats(
                "Exporting...", "Refining Hair System %s" % psys.name
            )
        ...
        if is_for_duplication:
            # We have to unapply the transformation which is baked into the
            # Blender hair coordinates
            transformation = utils.luxutils.matrix_to_list(
                obj.matrix_world, invert=True
            )
        else:
            transformation = None
        ...
        success = superluxcore_scene.DefineBlenderStrands(
            ...
            width_offset,
            settings.tesseltype,
            settings.adaptive_maxdepth,
            settings.adaptive_error,
            settings.solid_sidecount,
            settings.solid_capbottom,
            settings.solid_captop,
            list(settings.root_color),
            list(settings.tip_color),
        )

        # Sometimes no hair shape could be created, e.g. if the length
        # of all hairs is 0 (can happen e.g. during animations or if hair
        # length is textured)
        if not success:
            return None
...
def convert_hair_curves(
    ...
    if (
        radius_attribute is not None
        and radius_attribute.domain == "POINT"
        and radius_attribute.data_type == "FLOAT"
        and len(radius_attribute.data) == points.size // 3
    ):
        radii = np.empty(len(radius_attribute.data), dtype=np.float32)
        radius_attribute.data.foreach_get("value", radii)
        strand_ends = np.cumsum(points_per_strand)
        nonempty_strands = points_per_strand > 0
        if np.any(nonempty_strands):
            root_radius = float(
                radii[(strand_ends - points_per_strand)[nonempty_strands]].mean()
            )
            tip_radius = float(
                radii[(strand_ends - 1)[nonempty_strands]].mean()
            )
            if root_radius > 0:
                strand_diameter *= root_radius
                tip_width *= tip_radius / root_radius
            else:
                strand_diameter *= float(radii.mean())
    ...
    if export_color != "none" or uvs_needed:
        ...
        if uvs_needed:
            # The attribute is not guaranteed to exist, e.g. on curves
            # generated by geometry nodes or parented manually
            surface_uv_attribute = obj.data.attributes.get(
                "surface_uv_coordinate"
            )
            if surface_uv_attribute is not None:
                uvs = np.fromiter(
                    (
                        elem
                        for uv_coord in surface_uv_attribute.data
    ...
    if len(uvs) == 0:
        copy_uvs = False
        # DefineBlenderCurveStrands throws if an image is provided without UVs
        image_filename = ""
