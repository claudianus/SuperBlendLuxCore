...
You can create a symbolic link, for example via `ln -s /PATH/TO/SuperBlendLuxCore/debug-helpers/blendev/blendev ~/.local/bin/blendev`,
or directly add `/path/to/SuperBlendLuxCore/debug-helpers/blendev/blendev` to your PATH environment variable so that the command is always available.
