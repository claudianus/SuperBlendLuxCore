...
AOVS = {
    ...
    "MATERIAL_ID": ConvertFilmChannelOutput(1, np.uint32, 1, is_id=True, normalize=False),
    "OBJECT_ID": ConvertFilmChannelOutput(1, np.uint32, 1, is_id=True, normalize=False),
...
class FrameBufferFinal:
        ...
        if not engine.is_preview:
            ...
            for i, name in enabled_lightgroups:
                ...
                try:
                    ...
                    print(
                        "Error on import of Lightgroup AOV of group "
                        f"{name}: {error}"
                    )
    ...
    def _refresh_denoiser(
        ...
        if refresh_denoised:
            ...
            denoiser_pipeline_props = get_denoiser_imgpipeline_props(
                None, scene, denoiser_pipeline_index
            )
