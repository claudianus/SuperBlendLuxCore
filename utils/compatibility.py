...
def run():
    ...
    for camera in bpy.data.cameras:
        if camera.superluxcore.use_dof:
...
def update_mat_output_volume_change(node_tree):
    ...
    for old_output in find_nodes(node_tree, "SuperLuxCoreNodeMatOutput", False):
        ...
        new_out = node_tree.nodes.new("SuperLuxCoreNodeMatOutput")
        ...
        try:
            ...
            if interior:
                volume_pointer = node_tree.nodes.new("SuperLuxCoreNodeTreePointer")
...
def update_glossy_ior_change(node_tree):
    ...
    affected_nodes = find_nodes(node_tree, "SuperLuxCoreNodeMatGlossy2", False)
    affected_nodes += find_nodes(node_tree, "SuperLuxCoreNodeMatGlossyCoating", False)

    for node in affected_nodes:
        node_id = node.inputs.find("IOR")
        if node_id == -1:
            # Note: the IOR input will be at the very bottom, but at least the export works
            node.add_input("SuperLuxCoreSocketIOR", "IOR", 1.5)
...
def update_volume_asymmetry_change(node_tree):
    ...
    affected_nodes += find_nodes(node_tree, "SuperLuxCoreNodeVolHomogeneous", False)
    ...
    for node in affected_nodes:
        ...
        if asymmetry_socket.bl_idname == "NodeSocketUndefined":
            ...
            node.add_input("SuperLuxCoreSocketVolumeAsymmetry", "Asymmetry", (0, 0, 0))
            print('Updated %s node "%s" in tree "%s" to new version' % (node.bl_idname, node.name, node_tree.name))
...
def update_colormix_remove_min_max_sockets(node_tree):
    ...
    for node in find_nodes(node_tree, "SuperLuxCoreNodeTexColorMix", False):
...
def update_imagemap_remove_gamma_brightness_sockets(node_tree):
    ...
    for node in find_nodes(node_tree, "SuperLuxCoreNodeTexImagemap", False):
        updated = False
...
def update_cloth_remove_repeat_sockets(node_tree):
    ...
    for node in find_nodes(node_tree, "SuperLuxCoreNodeMatCloth", False):
...
def update_imagemap_add_alpha_output(node_tree):
    ...
    for node in find_nodes(node_tree, "SuperLuxCoreNodeTexImagemap", False):
        ...
        if node_id == -1:
            node.outputs.new("SuperLuxCoreSocketFloatUnbounded", "Alpha")
            print('Updated %s node "%s" in tree "%s" to new version' % (node.bl_idname, node.name, node_tree.name))
...
def update_smoke_multiple_output_channels(node_tree):
    ...
    for node in find_nodes(node_tree, "SuperLuxCoreNodeTexSmoke", False):
        node_id = node.outputs.find("density")
        if node_id != -1:
            continue

        node.outputs.new("SuperLuxCoreSocketFloatPositive", "density")
        node.outputs.new("SuperLuxCoreSocketFloatPositive", "fire")
        node.outputs.new("SuperLuxCoreSocketFloatPositive", "heat")
        node.outputs.new("SuperLuxCoreSocketColor", "color")
        node.outputs.new("SuperLuxCoreSocketColor", "velocity")
...
def update_smoke_mantaflow_simulation(node_tree):
    ...
    for node in find_nodes(node_tree, "SuperLuxCoreNodeTexSmoke", False):
        ...
        node.outputs.new("SuperLuxCoreSocketFloatPositive", "density")
        node.outputs.new("SuperLuxCoreSocketFloatPositive", "flame")
        node.outputs.new("SuperLuxCoreSocketFloatPositive", "heat")
        node.outputs.new("SuperLuxCoreSocketFloatPositive", "temperature")
        node.outputs.new("SuperLuxCoreSocketColor", "color")
        node.outputs.new("SuperLuxCoreSocketColor", "velocity")
...
def update_mat_output_add_shape_input(node_tree):
    ...
    for node in find_nodes(node_tree, "SuperLuxCoreNodeMatOutput", False):
        node_id = node.inputs.find("Shape")
        if node_id == -1:
            node.inputs.new("SuperLuxCoreSocketShape", "Shape")
...
def update_glass_disney_add_film_sockets(node_tree):
    affected_nodes = find_nodes(node_tree, "SuperLuxCoreNodeMatGlass", False)
    affected_nodes += find_nodes(node_tree, "SuperLuxCoreNodeMatDisney", False)
...
def update_invert_add_maximum_input(node_tree):
    ...
    for node in find_nodes(node_tree, "SuperLuxCoreNodeTexInvert", False):
        node_id = node.inputs.find("Maximum")
        if node_id == -1:
            node.add_input("SuperLuxCoreSocketFloatPositive", "Maximum", 1)
...
def update_brick_texture(node_tree):
    ...
    for node in find_nodes(node_tree, "SuperLuxCoreNodeTexBrick", False):
