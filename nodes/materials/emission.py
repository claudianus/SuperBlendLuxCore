...
from ... import icons, utils, properties, export
from .. import base
...
DLS_AUTO_DESC = (
    "Direct light sampling is disabled if the mesh has more than 256 triangles"
)
DLS_ENABLED_DESC = (
...
class SuperLuxCoreNodeMatEmission(base.SuperLuxCoreNode, bpy.types.Node):
    ...
    emission_units = [
        (
            "artistic",
            "Artistic",
            "Artist friendly unit using Gain and Exposure",
            0,
        ),
    ...
    emission_unit: EnumProperty(
        update=utils.node.force_viewport_update,
        name="Unit",
        items=emission_units,
        default="artistic",
    )
    gain: FloatProperty(
        update=utils.node.force_viewport_update,
    ...
    candela: FloatProperty(
        ...
        default=80,
        min=0,
        description=CANDELA_DESCRIPTION,
    )
    ...
    dls_type: EnumProperty(
        update=utils.node.force_viewport_update,
        name="DLS",
        description="Direct Light Sampling Type",
        items=dls_type_items,
        default="AUTO",
    )
    ...
    def export_emission(self, exporter, depsgraph, props, definitions):
        ...
        if self.emission_unit == "power":
            definitions["emission.power"] = self.power / (
                2 * math.pi * (1 - math.cos(self.spread_angle / 2))
            )
            ...
            definitions["emission.power"] = self.lumen / (
                2 * math.pi * (1 - math.cos(self.spread_angle / 2))
            )
            ...
            if self.per_square_meter:
                ...
                definitions["emission.gain.normalizebycolor"] = (
                    self.normalizebycolor
                )
            else:
                definitions["emission.power"] = self.candela * math.pi
                definitions["emission.efficency"] = 1.0
                definitions["emission.normalizebycolor"] = (
                    self.normalizebycolor
                )
                if self.candela == 0:
                    definitions["emission.gain"] = [0, 0, 0]
            ...
            definitions["emission.efficency"] = 0
            definitions["emission.gain"] = [
                self.gain * pow(2, self.exposure)
            ] * 3
