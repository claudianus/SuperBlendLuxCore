"""
Pure-GPU host spilling regression test (PATHOCL / Metal, no native threads).

    Blender -b --python dev-tools/puregpu_spill_test.py

Same scene as stagingspill_test.py but with use_native_cpu=False, so
nativeRenderThreadCount==0 and every accelerator's host node array
(including the would-be-native one) is eligible for spilling.

What it verifies:
  * "Accelerator host nodes spilled to disk" appears even though the
    accelerator is the only one in the scene (keepHot == ACCEL_AUTO).
  * Host staging + geometry + image-map spills all fire.
  * The render result is correct on a pure GPU path.
"""
...
def make_image():
    n = 1024
    path = "/tmp/puregpu_spill_tex.png"
...
def main():
    ...
    print("[PureGPUSpillTest] Saved:", OUT)
